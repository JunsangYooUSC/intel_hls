...
// __HLS_CONSTEXPR__ is defined as constexpr as long as no DEBUG flag is
// enabled.
#if defined(DEBUG_AC_INT_ERROR) || defined(DEBUG_AC_INT_WARNING)
#define __HLS_CONSTEXPR__
#pragma message ("using ac_int debug macros (DEBUG_AC_INT_WARNING/DEBUG_AC_INT_ERROR) may result in performance degradation when compiling for FPGA.")
#else
#define __HLS_CONSTEXPR__ constexpr
#define __HLS_USE_CONSTEXPR__
#endif
...
namespace ac_private {
  ...
  template <int N>
  constexpr ap_uint<N> bit_division(ap_uint<N> value, ap_uint<N> divisor, ap_uint<N> & remainder) {
  ...
  public:
    ...
    constexpr Slong to_int64() const {
      return (Slong) value;
    }
    constexpr Ulong to_uint64() const {
      return (Ulong) value;
    }
    ...
    template<int N2, bool S2>
    __HLS_CONSTEXPR__ void debug_within_range(const iv<N2, S2> &op2){
    #if !defined(__HLS_USE_CONSTEXPR__)
    ...
    __HLS_CONSTEXPR__ void debug_within_range(Ulong v){
      debug_within_range(iv<64, false>(v));
    }

    __HLS_CONSTEXPR__ void debug_within_range(Slong v){
    ...
    template<int N2, bool S2, int Nr, bool Sr>
    constexpr void rem(const iv<N2, S2> &op2, iv<Nr, Sr> &r) const {
      r.value = value;
      r.value %= op2.value;
    }
...
public:
  ...
  __HLS_CONSTEXPR__ ac_int() {
    #if !defined(__HLS_USE_CONSTEXPR__)
      if(!warned_undef) {
        printf("warning: using empty constructor for type %s\n", type_name().c_str());
        warned_undef = true;
        #ifdef DEBUG_AC_INT_ERROR
        AC_ASSERT(0, "Assert due to using empty constructor (DEBUG_AC_INT_ERROR)\n");
        #endif
      }
    #endif
  }
  template<int W2, bool S2>
  __HLS_CONSTEXPR__ inline ac_int (const ac_int<W2,S2> &op) : ConvBase(op) {
    Base::debug_within_range(op);
  }

  __HLS_CONSTEXPR__ inline ac_int( bool b ) : ConvBase(b) { }
  __HLS_CONSTEXPR__ inline ac_int( char b ) : ConvBase(b) {
    Base::debug_within_range(Ulong(b));
  }
  __HLS_CONSTEXPR__ inline ac_int(signed char b) : ConvBase(b) {
    Base::debug_within_range(Slong(b));
  }
  __HLS_CONSTEXPR__ inline ac_int(unsigned char b) : ConvBase(b) {
    Base::debug_within_range(Ulong(b));
  }
  __HLS_CONSTEXPR__ inline ac_int(signed short b) : ConvBase(b) {
    Base::debug_within_range(Slong(b));
  }
  __HLS_CONSTEXPR__ inline ac_int(unsigned short b) : ConvBase(b) {
    Base::debug_within_range(Ulong(b));
  }
  __HLS_CONSTEXPR__ inline ac_int(signed int b) : ConvBase(b) {
    Base::debug_within_range(Slong(b));
  }
  __HLS_CONSTEXPR__ inline ac_int( unsigned int b ) : ConvBase(b) {
    Base::debug_within_range(Ulong(b));
  }
  __HLS_CONSTEXPR__ inline ac_int( signed long b ) : ConvBase(b) {
    Base::debug_within_range(Slong(b));
  }
  __HLS_CONSTEXPR__ inline ac_int( unsigned long b ) : ConvBase(b) {
    Base::debug_within_range(Ulong(b));
  }
  __HLS_CONSTEXPR__ inline ac_int( Slong b ) : ConvBase(b) {
    Base::debug_within_range(b);
  }
  __HLS_CONSTEXPR__ inline ac_int( Ulong b ) : ConvBase(b) {
    Base::debug_within_range(b);
  }
...
#pragma clang diagnostic ignored "-Wuninitialized"
  ...
  __HLS_CONSTEXPR__ inline ac_int &set_val() {
    if(V == AC_VAL_DC) {
      ac_int r = 0;
      Base::operator =(r);
    }
...
#pragma clang diagnostic pop
  ...
  constexpr unsigned long to_ulong() const { return (unsigned long) Base::value; }
  ...
  // Arithmetic : Binary ----------------------------------------------------
  template<int W2, bool S2>
  __HLS_CONSTEXPR__ typename rt<W2,S2>::mult operator *( const ac_int<W2,S2> &op2) const {
    typename rt<W2,S2>::mult r = 0;
    Base::mult(op2, r);
    return r;
  }
  template<int W2, bool S2>
  __HLS_CONSTEXPR__ typename rt<W2,S2>::plus operator +( const ac_int<W2,S2> &op2) const {
    typename rt<W2,S2>::plus r = 0;
    Base::add(op2, r);
    return r;
  }
  template<int W2, bool S2>
  __HLS_CONSTEXPR__ typename rt<W2,S2>::minus operator -( const ac_int<W2,S2> &op2) const {
    typename rt<W2,S2>::minus r = 0;
    Base::sub(op2, r);
    return r;
  }
  template<int W2, bool S2>
  __HLS_CONSTEXPR__ typename rt<W2,S2>::div operator /( const ac_int<W2,S2> &op2) const {
    typename rt<W2,S2>::div r = 0;
    Base::div(op2, r);
    return r;
  }
  template<int W2, bool S2>
  __HLS_CONSTEXPR__ typename rt<W2,S2>::mod operator %( const ac_int<W2,S2> &op2) const {
  ...
  template<int W2, bool S2>
  __HLS_CONSTEXPR__ void check_overflow(const ac_int<W2, S2> &op2, ac_debug_op debug_op){
    #if !defined(__HLS_USE_CONSTEXPR__)
    ac_int<W, S> temp = 0;
    switch (debug_op){
      case AC_DEBUG_ADD:
  ...
  __HLS_CONSTEXPR__ void check_overflow(ac_debug_op debug_op){
    #if !defined(__HLS_USE_CONSTEXPR__)
    ac_int<W, S> temp = 0;
    ac_int<2, true> op2 = 1;
    switch (debug_op){
  ...
  // Arithmetic assign  ------------------------------------------------------
  template<int W2, bool S2>
  __HLS_CONSTEXPR__ ac_int &operator *=(const ac_int<W2,S2> &op2) {
  ...
  __HLS_CONSTEXPR__ ac_int &operator ++() {
  ...
  __HLS_CONSTEXPR__ ac_int operator ++(int) {
    check_overflow(AC_DEBUG_INCREMENT);
    ac_int t = *this;
    Base::increment();
    return t;
  ...
  __HLS_CONSTEXPR__ typename rt_unary::neg operator -() const {
  ...
  __HLS_CONSTEXPR__ ac_int<W+!S, true> operator ~() const {
    ac_int<W+!S, true> r = 0;
    Base::bitwise_complement(r);
    return r;
  }
  // Bitwise (non-arithmetic) bit_complement  -----------------------------
  __HLS_CONSTEXPR__ ac_int<W, false> bit_complement() const {
    ac_int<W, false> r = 0;
    Base::bitwise_complement(r);
    return r;
  }
  ...
  template<int W2, bool S2>
  __HLS_CONSTEXPR__ typename rt<W2,S2>::logic operator & ( const ac_int<W2,S2> &op2) const {
  ...
  template<int W2>
  __HLS_CONSTEXPR__ ac_int operator << ( const ac_int<W2,true> &op2 ) const {
    ac_int r = 0;
    Base::shift_l2(op2.to_int(), r);
    return r;
  }
  template<int W2>
  __HLS_CONSTEXPR__ ac_int operator << ( const ac_int<W2,false> &op2 ) const {
    ac_int r = 0;
  ...
  // Shift assign ------------------------------------------------------------
  template<int W2>
  __HLS_CONSTEXPR__ ac_int &operator <<= ( const ac_int<W2,true> &op2 ) {
    Base r = 0;
    Base::shift_l2(op2.to_int(), r);
    Base::operator=(r);
    return *this;
  ...
  template<int W2>
  __HLS_CONSTEXPR__ ac_int &operator <<= ( const ac_int<W2,false> &op2 ) {
    Base r = 0;
    Base::shift_l(op2.to_uint(), r);
    Base::operator=(r);
    return *this;
  ...
  // Relational ---------------------------------------------------------------
  template<int W2, bool S2>
  __HLS_CONSTEXPR__ bool operator == ( const ac_int<W2,S2> &op2) const {
  ...
  public:
    ...
    operator bool () const {
      return (d_index < W) ? (bool)(d_bv.value>>(d_index) & 1) : 0;
    }
...
#define BIN_OP_WITH_INT(BIN_OP, C_TYPE, WI, SI, RTYPE)  \
  template<int W, bool S> \
  __HLS_CONSTEXPR__ inline typename ac_int<WI,SI>::template rt<W,S>::RTYPE operator BIN_OP ( C_TYPE i_op, const ac_int<W,S> &op) {  \
    return ac_int<WI,SI>(i_op).operator BIN_OP (op);  \
  } \
  template<int W, bool S>   \
  __HLS_CONSTEXPR__ inline typename ac_int<W,S>::template rt<WI,SI>::RTYPE operator BIN_OP ( const ac_int<W,S> &op, C_TYPE i_op) {  \
    return op.operator BIN_OP (ac_int<WI,SI>(i_op));  \
  }

#define REL_OP_WITH_INT(REL_OP, C_TYPE, W2, S2)  \
  template<int W, bool S>   \
  __HLS_CONSTEXPR__ inline bool operator REL_OP ( const ac_int<W,S> &op, C_TYPE op2) {  \
    return op.operator REL_OP (ac_int<W2,S2>(op2));  \
  }  \
  template<int W, bool S> \
  __HLS_CONSTEXPR__ inline bool operator REL_OP ( C_TYPE op2, const ac_int<W,S> &op) {  \
    return ac_int<W2,S2>(op2).operator REL_OP (op);  \
  }

#define ASSIGN_OP_WITH_INT(ASSIGN_OP, C_TYPE, W2, S2)  \
  template<int W, bool S>   \
  __HLS_CONSTEXPR__ inline ac_int<W,S> &operator ASSIGN_OP ( ac_int<W,S> &op, C_TYPE op2) {  \
